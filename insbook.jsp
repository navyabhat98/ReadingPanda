...
.navbar-right {
    ...
    <div class="LoginForm">   
            <div class="container">
                ...
                <div class="login-form">
                    <div class="main-div">
                        <div class="panel">
                            <h1  style="font-family:cursive;"><font color="#ff3333">INSERT BOOK</font></h1>
                         </div>
        	<form id="Login" name="f1" method="POST" action="insbook.jsp">
        	<div class="form-group">
           <font color="#ff3333"> BOOK ID:</font>  <input type="text" name="B_ID" value=""> <br>
            </div>
            <div class="form-group">
            <font color="#ff3333">BOOK NAME:</font> <input type="text" name="BOOK_NAME" value=""> <br>
            </div>
            <div class="form-group">
            <font color="#ff3333">E-LINK:</font> <input type="text" name="E_LINK" value=""> <br>
            </div>
            ...
            String E_LINK = request.getParameter("E_LINK");
           ...
           ResultSet result3=null;
            ...
            result1 = statement1.executeQuery("call ps1()");
            result2 = statement2.executeQuery("select * from language");
            result3 = statement3.executeQuery("select * from genre") ;          
                      
        %>
       
    <font color="#ff3333">AUTHOR ID:</font>
        <select name="A_ID">
        <%  while(result1.next()){ %>
            <option><%= result1.getString("A_ID")%></option>
        <% } %>
        </select>
        <br>
        <br>
        <font color="#ff3333"> LANGUAGE ID:</font>
        <select name="L_ID">
        <%  while(result2.next()){ %>
        
            <option><%= result2.getString("L_ID")%></option>
        <% } %>
        </select>
        <br>
        <br>
        <font color="#ff3333"> GENRE ID:</font>
        <select name="G_ID">
        <%  while(result3.next()){ %>
            <option><%= result3.getString("G_ID")%></option>
        <% } %>
        </select>
        <br>
        <br>
        ...
        <%  
        String s1=request.getParameter("A_ID");
        String s2=request.getParameter("L_ID");
        String s3=request.getParameter("G_ID");
  if(B_ID!=null && BOOK_NAME!=null && s1!=null && s2!=null && s3!=null) {
	  try{
    				   PreparedStatement ps1 = null;
					   ps1 = con.prepareStatement("INSERT INTO BOOK VALUES(?,?,?,?,?,?)");
		               ps1.setString(1, B_ID);    //set the first paremter to language ID
		               ps1.setString(2, BOOK_NAME);
		               ps1.setString(3,E_LINK);
		               ps1.setString(4,s1);
		               ps1.setString(5,s2);
		               ps1.setString(6,s3);
		               ...
		               ps2 = con.prepareStatement("INSERT INTO WRITTENBY VALUES(?,?)");
		               ps2.setString(1,s1);
		               ps2.setString(2,B_ID);
		               int b = ps2.executeUpdate();
		               
		               PreparedStatement ps3 = null;
		               ps3 = con.prepareStatement("INSERT INTO CATEGORIZES VALUES(?,?)");
		               ps3.setString(1,s1);
		               ps3.setString(2,s3);
 ...
 %>
 			...
 			function fun() {
 			    alert("Time stamp stored");
 			}</script>
 			

 			
            <input type="submit" value="insert" onclick="fun()">
        </form>
            </div>
            </div>
            </div>
...
ResultSet rs2=st2.executeQuery("select * from mood");
